...
def objective(trial):
    try:
        ...
        gan_lr = trial.suggest_float('gan_lr', 1e-5, 1e-3)
        critic_lr = trial.suggest_float('critic_lr', 1e-5, 1e-3)
        n_critic = trial.suggest_int('n_critic', 1, 25)
        clip_value = trial.suggest_float('clip_value', 0.009, 0.1)
        num_lstm = trial.suggest_int('num_lstm', 1, 5)
